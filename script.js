...
function syncMiniPlayIcon() {
  const isPaused = ui.audio.paused;
  ui.miniPlayPauseBtn.innerHTML = `<i data-lucide="${isPaused ? "play" : "pause"}"></i>`;
  renderIcons();
}
...
function setMood(mood) {
  ...
  ui.playlistTitle.innerHTML = `<i data-lucide="music"></i> ${mood.charAt(0).toUpperCase()}${mood.slice(1)} Playlist`;
  renderIcons();
...
function playNext() {
  ...
  if (state.isShuffle) {
    let random = Math.floor(Math.random() * state.currentPlaylist.length);
    if (state.currentPlaylist.length > 1 && random === state.currentIndex) {
      random = (random + 1) % state.currentPlaylist.length;
    }
...
ui.miniPlayPauseBtn.addEventListener("click", async () => {
  setupAudioContext();
  if (audioCtx.state === "suspended") await audioCtx.resume();
  if (ui.audio.paused) {
    await ui.audio.play();
