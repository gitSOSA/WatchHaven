...
function load() {
  ...
  if (urlData) {
    const decoded = safeDecode(urlData);
...
function finalizeAddMovie(movie) {
  ...
  state.movies.unshift(normalizeMovie(movie));
  persist();
  renderMovies();
  fetchRecommendations();
...
function shareAsLink() {
  ...
  const payload = btoa(encodeURIComponent(JSON.stringify(state.movies)));
  const url = `${window.location.origin}${window.location.pathname}?wl=${payload}`;
  navigator.clipboard?.writeText(url).catch(() => {});
  els.shareMessage.textContent = `Share link copied (or shown here): ${url}`;
}
